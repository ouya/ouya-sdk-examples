...
import tv.ouya.examples.android.virtualcontrollerappwithnative.MappingParser.AxisRemap;
import tv.ouya.examples.android.virtualcontrollerappwithnative.MappingParser.ButtonIsAxis;
...
public class OuyaActivity extends Activity {
    ...
    static {
    	Log.i(TAG, "Loading VirtualControllerAppWithNative...");
        System.loadLibrary("VirtualControllerAppWithNative");
    }
		...
		*/
			...
			for (int i = 0; i < buttons.size(); ++i) {
				...
				if (axis == button.mActionDown) {
					if (!mLastValue.containsKey(button.mDestinationKeyCode) ||
						...
						dispatchKeyEventNative(deviceId, code, action);
					}
				} else {
					if (mLastValue.containsKey(button.mDestinationKeyCode) &&
						mLastValue.get(button.mDestinationKeyCode)) {					
	    ...
	    if (null != axises) {	    
		    ...
		    if (pointerCount > 0 &&
		    	...
		    	dispatchGenericMotionEventNative(deviceId, OuyaController.AXIS_LS_X, motionEvent.getAxisValue(OuyaController.AXIS_LS_X));
			    dispatchGenericMotionEventNative(deviceId, OuyaController.AXIS_LS_Y, motionEvent.getAxisValue(OuyaController.AXIS_LS_Y));	    
			    dispatchGenericMotionEventNative(deviceId, OuyaController.AXIS_RS_X, motionEvent.getAxisValue(OuyaController.AXIS_RS_X));
			    dispatchGenericMotionEventNative(deviceId, OuyaController.AXIS_RS_Y, motionEvent.getAxisValue(OuyaController.AXIS_RS_Y));
			    dispatchGenericMotionEventNative(deviceId, OuyaController.AXIS_L2, motionEvent.getAxisValue(OuyaController.AXIS_L2));
			    dispatchGenericMotionEventNative(deviceId, OuyaController.AXIS_R2, motionEvent.getAxisValue(OuyaController.AXIS_R2));
		    	motionEvent.recycle();
		    	return true;
		    }
	    }
