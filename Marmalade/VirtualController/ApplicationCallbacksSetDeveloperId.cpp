...
void SetDeveloperIdOnSuccess(s3eSetDeveloperIdSuccessEvent* event)
{
	...
	if (event)
	{
		Application::m_callbacksSetDeveloperId->OnSuccess();
	}
	else
	{
		Application::m_callbacksSetDeveloperId->OnFailure(-1, "Success event is null");
	}
}
...
void SetDeveloperIdOnFailure(s3eSetDeveloperIdFailureEvent* event)
{
	...
	if (event)
	{
		Application::m_callbacksSetDeveloperId->OnFailure(event->m_errorCode, event->m_errorMessage);
	}
	else
	{
		Application::m_callbacksSetDeveloperId->OnFailure(-1, "Failure event is null");
	}
}
