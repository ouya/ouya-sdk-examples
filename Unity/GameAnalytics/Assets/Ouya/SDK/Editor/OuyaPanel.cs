...
public class OuyaPanel : EditorWindow
{
    ...
    private static string pathGAJar = string.Empty;
    ...
    void UpdateOuyaPaths()
    {
        ...
        pathGAJar = string.Format("{0}/Assets/Plugins/Android/libs/GA_Android.jar", pathUnityProject);
    ...
    static bool CompileApplicationClasses()
    {
        ...
        if (File.Exists(pathGAJar))
        {
            Debug.Log(string.Format("Found GA jar: {0}", pathGAJar));
        }
        else
        {
            Debug.LogError(string.Format("Failed to find GA jar: {0}", pathGAJar));
            return false;
        }
        ...
        {
            case RuntimePlatform.OSXEditor:
                jars = string.Format("\"{0}:{1}:{2}:{3}:{4}:{5}:{6}\"",
                    pathToolsJar, GetPathAndroidJar(), pathGsonJar, pathUnityJar, pathOuyaUnityPluginJar, pathOuyaSDKJar, pathGAJar);
        ...
        switch (m_selectedToolSet)
        {
            case 0:
                ...
                */
                ...
                GUIDisplayUnityFile("GA JAR", pathGAJar);
